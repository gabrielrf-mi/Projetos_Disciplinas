    ...
    if(socket->state() == QAbstractSocket::ConnectedState){
        ...
        if(socket->isOpen()){
            ...
            for (int i = 0; i < thetime2.size(); ++i) {
                tempo.push_back((float)(QString::number(thetime2[i]).toLongLong(&ok,10)- QString::number(thetime2[0]).toLongLong(&ok,10))/
                        (QString::number(thetime2[thetime2.size()-1]).toLongLong(&ok,10)- QString::number(thetime2[0]).toLongLong(&ok,10)));
